...
from streamvln.selfcorrect.deviation_judge import TrajectoryRecorder
class VLNEvaluator:
    def __init__(
        self,
        ...
        if args is not None and getattr(args, 'device', None) is not None:
            ...
            self.device = next(model.parameters()).device if model is not None else torch.device('cuda:0')
        ...
        if self.max_latent_num>=0:
            ...
            print(f"=====================================latent mode===================================== Using latent_num {self.max_latent_num} due to MAX_LATENT_NUM env var.")
        ...
        self.conav_expert_model = os.environ.get("CONAV_EXPERT_MODEL", "")
        ...
        print(f"[VLNEvaluator] inference device: {self.device}")
        if self.use_conav_expert:
            print(f"[VLNEvaluator] conav=1, expert API enabled: {self.conav_expert_api}")
    ...
    def preprocess_qwen(self, sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False, max_len=2048, system_message: str = "You are a helpful assistant.",add_system: bool = False, expert_info: str = ""):
        ...
        for i, source in enumerate(sources):
            ...
            if expert_info:
                source[0]["value"] += f' You can also access information from experts of spatial understanding: {expert_info}'
    ...
    def _query_conav_expert(self, image: Image.Image):
        ...
        try:
            ...
            if self.conav_expert_max_chars > 0:
                txt = txt[: self.conav_expert_max_chars]
            return txt
        except Exception as e:
            if not self._printed_expert_error:
                print(f"[VLNEvaluator][conav] expert query failed once: {repr(e)}")
                self._printed_expert_error = True
            return ""

    def _build_conav_expert_info(self, rgb):
        if not self.use_conav_expert:
            return ""
        image = Image.fromarray(rgb).convert('RGB')
        cache_key = f"shape={rgb.shape}|sum={int(np.asarray(rgb, dtype=np.uint8).sum())}"
        if cache_key in self._conav_expert_cache:
            return self._conav_expert_cache[cache_key]
        txt = self._query_conav_expert(image)
        self._conav_expert_cache[cache_key] = txt
        return txt
    ...
    def step(self, idx, rgb, instruction_text='', run_model=False):
        ...
        expert_info = ""
        if self.use_conav_expert:
            expert_info = self._build_conav_expert_info(rgb)
            print(f"[VLNEvaluator][conav][3d_expert] {expert_info}")
        ...
        if self.output_ids is None:
            ...
            if self.max_latent_num >= 0:
                sources[0]["value"] += f' You can also access to latent observations{DEFAULT_LATENT_TOKEN*self.latent_max_latentpadNum} if needed.'
        ...
        input_ids, conversations = self.preprocess_qwen([sources], self.tokenizer, True, add_system=add_system, expert_info=expert_info)
