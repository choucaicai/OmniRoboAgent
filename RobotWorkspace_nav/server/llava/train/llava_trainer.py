...
class LLaVATrainer(Trainer):
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            if self.args.latent_proj_lr is not None:          # ← 新增
                lr_mapper["latent_proj"] = self.args.latent_proj_lr  # ← 新增
            if len(lr_mapper) > 0:
    ...
    #     # 1) pop 出权重，避免传给 model.forward
    #     loss_weights = inputs.pop("loss_weights", None)
    #     if loss_weights is None:
    #         loss_weights = inputs.pop("weights", None)

    #     # 2) 复用默认 compute_loss（它会用模型内部正确的对齐/shift/ignore_index 逻辑算出标量 loss）
    #     loss, outputs = super().compute_loss(model, inputs, return_outputs=True)

    #     # if os.getenv("debug_flag", "False") == "debug_loss": 
    ...
    #     # 4) 把 loss_weights 规约成“每个样本一个标量权重”
    #     #    你现在 loss_weights 常见是 [B, L]，但你需求是 step-level weight（标量）
    ...
    #     # 情况 A：w 是 [B, L]（你现在就是这样）
    #     if w.dim() == 2:
    #         if labels is not None and torch.is_tensor(labels) and labels.shape == w.shape:
    #             # 用 labels != -100 的位置做 mask，把“有效监督 token”的权重平均成一个标量
    #             # 如果你的每个 step 只有一个 action token 被监督（只有一个非 -100），那这一步等价于取该 action token 的权重
    #             mask = (labels != IGNORE_INDEX).to(w.dtype)      # [B, L]
    #             denom = mask.sum(dim=1).clamp_min(1.0)           # [B]
    #             w_per_ex = (w * mask).sum(dim=1) / denom         # [B]
    #         else:
    #             # 没有 labels 或形状不匹配：退化为对序列取均值
    #             w_per_ex = w.mean(dim=1)                         # [B]

    #         # batch 里通常每个样本一个 step；如果 B>1 且你希望不同样本不同权重：
    #         # 注意：super().compute_loss 返回的是“整个 batch 合并后的一个标量”，无法精确做 per-example 加权
    #         # 最稳妥做法是把 batch_size 设为 1；否则这里只能用均值近似缩放整个 batch 的梯度
    #         scale = w_per_ex.mean()
    ...
    #     else:
    #         # 其它奇怪形状，尽量规约成标量
    #         scale = w.float().mean().to(loss.dtype)

    #     # 5) step-level 缩放 loss（整体缩放梯度）
    #     loss = loss * scale

    #     # 6) 更新 outputs.loss（可选，但对 logging/某些 Trainer 逻辑更一致）
    #     try:
    #         outputs.loss = loss
    #     except Exception:
    #         pass
