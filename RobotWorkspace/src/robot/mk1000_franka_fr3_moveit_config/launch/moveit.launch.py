...
def create_arm_robot_moveit_nodes(
    ...
    rviz_node = Node(
        ...
        condition=IfCondition(use_rviz) if use_rviz is not None else None,
    )
    rviz_delayed = TimerAction(period=8.0, actions=[rviz_node])
    return [run_move_group_node, rviz_delayed]
...
def generate_launch_description():
    use_rviz_arg = DeclareLaunchArgument(
        ...
        default_value="false",
        choices=["true", "false"],
        description="Whether to start RViz for MoveIt visualization"
    )
    arm_robot_nodes = create_arm_robot_nodes(
        arm_id="fr3", arm_prefix="mk1000", robot_namespace="pc_arm"
    )
    robot_moveit_nodes = create_arm_robot_moveit_nodes(
        arm_id="fr3", arm_prefix="mk1000", robot_namespace="pc_arm", use_rviz=LaunchConfiguration("use_rviz")
    )

    return LaunchDescription([use_rviz_arg] + arm_robot_nodes + robot_moveit_nodes)
